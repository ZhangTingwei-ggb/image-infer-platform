...
def train_step(batch_data, run_info):
    ...
    train_dec_list = [
        ...
        if np.any([dec_head_name.replace('#', '-') in x for x in tgt_name_list])
    ]
    ...
    # * gradient update
    all_loss.backward()
    # clip gradients to avoid NaN from loss spikes (e.g. during fine-tuning)
