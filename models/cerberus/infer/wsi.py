...
def _rtree_query_indices(rtree, geometries, bounds, predicate=None):
    """STRtree.query wrapper that returns integer indices into `geometries`.

    shapely 2.0's STRtree.query returns an integer index array, whereas
    shapely 1.x returns the matching geometry objects.  This wrapper
    normalises the result to integer indices so both versions work.  The
    `predicate` kwarg is only supported in shapely 2.0; for 1.x it is
    emulated with the equivalent geometry predicate.
    """
    if predicate is None:
        ...
        try:
            ...
            # shapely 1.x has no predicate kwarg; emulate it manually
            res = rtree.query(bounds)
    ...
    # shapely 2.0: already integer indices
    if isinstance(res[0], (int, np.integer)):
        return [int(i) for i in res]

    # shapely 1.x: geometry objects -> map back to indices via id()
    id_lookup = {id(geo): i for i, geo in enumerate(geometries)}
    indices = []
    for geo in res:
        idx = id_lookup.get(id(geo))
        if idx is None:
            # fallback if shapely rebuilt the object: compare geometry
            idx = next(
                (i for i, g in enumerate(geometries) if g.equals(geo)), None
            )
        indices.append(idx)
    return indices
...
def _process_tile_predictions(
    ...
    if tile_mode in [0, 3]:
        ...
        sel_indices = [
            ...
            for idx in _rtree_query_indices(
                tile_rtree, geometries, bounds, predicate="contains"
            )
        ]
        ...
        sel_indices = [
            idx
            for bounds in sel_boxes
            for idx in _rtree_query_indices(tile_rtree, geometries, bounds)
        ]
    else:
    ...
    if tile_mode == 3:
        ...
        sel_indices = [
            ...
            for idx in _rtree_query_indices(ref_inst_rtree, geometries, bounds)
        ]
...
class InferManager(base.InferManager):
    ...
    def _get_resized_map(self, mmap_path, ds_factor, read_size):
        ...
        wsi_map_ptr = np.load(mmap_path, mmap_mode="r")
    ...
    def process_single_file(
        ...
        for tile_idx, tile_bounds in enumerate(set_bounds):
            ...
            sel_indices = _rtree_query_indices(spatial_indexer, geometries, sel_box)
            if len(sel_indices) == 0:
                continue
            ...
            for tissue_code in target_list:
                ...
                for output_type in ["INST", "TYPE"]:
                    if tissue_code + "-%s" % output_type in head_names:
                        ...
                        if tile_pred_map.shape[0] != mask_idx.shape[0] or tile_pred_map.shape[1] != mask_idx.shape[1]:
                            # resize tissue mask 
                            mask_idx = cv2.resize(
    ...
    def process_wsi_list(self, run_args):
        ...
        # Respect --nr_inference_workers / --nr_post_proc_workers from the CLI
        # (parsed into self by _parse_args). A value <= 0 falls back to the
        # previous defaults (12 loader / 6 post-proc workers).
        self.num_loader_workers = getattr(self, "nr_inference_workers", 0) or 12
        self.num_postproc_workers = getattr(self, "nr_post_proc_workers", 0) or 6

        # NOTE: inference and nuclei post-processing intentionally use
        # different tile sizes, so they stay as two separate CLI params.
        #   --chunk_shape  -> large inference tiles (network assembly)
        #   --tile_shape   -> smaller post-proc tiles (instance stitching
        #                     + margin-based duplicate removal is expensive)
        # patch_input/patch_output are fixed by the trained model (448/144)
        # and must NOT be changed, otherwise inference silently breaks.
        # getattr() fallbacks keep the previous hardcoded defaults if a
        # caller omits one of these keys from run_args.
        chunk_shape = getattr(self, "chunk_shape", [15000, 15000])
        tile_shape = getattr(self, "tile_shape", [4096, 4096])
        patch_input_shape = getattr(self, "patch_input_shape", [448, 448])
        patch_output_shape = getattr(self, "patch_output_shape", [144, 144])
        ambiguous_size = getattr(self, "ambiguous_size", 64)
        ...
        ioconfig = IOSegmentorConfig(
            ...
            margin=ambiguous_size,
            tile_shape=chunk_shape,
