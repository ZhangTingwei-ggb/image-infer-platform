...
class InferManager(base.InferManager):
    ...
    def process_file_list(self, run_args):
        ...
        # setup file logging (mirrors the WSI inference logging)
        os.makedirs(self.logging_dir, exist_ok=True)
        dt_now = datetime.now()
        dt_string = dt_now.strftime("%d-%m-%Y_%H-%M-%S")
        log_path = os.path.join(self.logging_dir, "tile_%s_std.log" % dt_string)
        self.logger = logging.getLogger("tile_infer")
        ...
        self.logger.info("Tile inference started")
        ...
        while len(file_path_list) > 0:
            ...
            while len(file_path_list) > 0:
                ...
                self.logger.info("Processing %s" % os.path.basename(file_path))
            ...
            for file_idx, file_info in enumerate(cache_image_info_list):
                ...
                if proc_pool is not None:
                    ...
                    self.logger.info("Done Assembling %s" % base_name)
            ...
            if proc_pool is not None:
                ...
                for future in as_completed(future_list):
                    ...
                    if future.exception() is not None:
                        self.logger.error("Silent Crash")
                    else:
                        proc_output = future.result()
                        file_path = proc_callback(proc_output, self.output_dir)
                        self.logger.info("Done Assembling %s" % proc_output[0])
        self.logger.info("Tile inference finished")
        self.logger.handlers.clear()
        return
